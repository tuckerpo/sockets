...
public:
    ...
    /**
     * Spin up a processing thread that will ::send and ::recv data. Calls dataCallback on data.
     * Calls brokenCallback if the connection dies.
     */
    virtual bool BeginProcessing(OnDataCallback dataCallback, OnBrokenCallback brokenCallback) override;
    /**
    ...
    /**
     * Return the remote address we are connected to currently, or zero if not connected.
     */
    ...
    virtual bool Write(const std::vector<uint8_t>&) override;
    /**

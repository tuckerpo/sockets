...
NetConnection::~NetConnection() {
  if (m_pPlatform->thread.joinable()) m_pPlatform->thread.join();
...
void NetConnection::NetRoutine() {
  ...
  std::unique_lock<decltype(m_pPlatform->mutex)> NetRoutineLock(
      m_pPlatform->mutex);
  while (m_pPlatform->sock != INVALID_SOCKET) {
    /* Recv. */
    rxBuf.resize(MAX_READ);
    const int numBytes =
        ::recv(m_pPlatform->sock, (char*)&rxBuf[0], (int)rxBuf.size(), 0);
    ...
    if (m_pPlatform->sock == INVALID_SOCKET) break;
    /* Send. */
    ...
    if (outQueueSize > 0) {
      ...
      const int bytesSent =
          ::send(m_pPlatform->sock, (const char*)&sendBuffer[0], sendSize, 0);
...
bool NetConnection::Connect(uint32_t peerAddr, uint16_t peerPort) {
  ...
  linger.l_onoff = 1;
  linger.l_linger = 0;
  (void)setsockopt(m_pPlatform->sock, SOL_SOCKET, SO_LINGER,
  ...
  if (::connect(m_pPlatform->sock, (const sockaddr*)&socketAddr,
                sizeof(socketAddr)) != 0) {
...
bool NetConnection::BeginProcessing(OnDataCallback dataCallback,
                                    OnBrokenCallback brokenCallback) {
  bool bResult = true;
  if (m_pPlatform->sock == INVALID_SOCKET) {
    bResult = false;
    return bResult;
  ...
  if (m_pPlatform->thread.joinable()) return bResult;
  m_dataCallback = dataCallback;
  m_onBrokenCallback = brokenCallback;
  m_pPlatform->thread = std::thread([this]() { this->NetRoutine(); });
  return bResult;
}
...
uint32_t NetConnection::GetPeerAddress() const {
  uint32_t addr = 0;
  if (m_pPlatform->sock != INVALID_SOCKET) addr = m_peerAddr;
  return addr;
}

uint16_t NetConnection::GetPeerPort() const { return m_peerPort; }

uint32_t NetConnection::GetBoundAddress() const { return m_boundAddr; }

uint16_t NetConnection::GetBoundPort() const { return m_boundPort; }

uint32_t NetConnection::GetAddressFromHost(const std::string& host) {
  bool wsaStarted = false;
  WSAData wsaData;
  wsaStarted = !WSAStartup(MAKEWORD(2, 0), &wsaData);
  ...
  /* Deferred free of addrinfo struct */
  std::unique_ptr<struct addrinfo, std::function<void(struct addrinfo*)> >
      results(rawResults, [](struct addrinfo* p) { freeaddrinfo(p); });
  if (results == NULL) {
    return 0;
  } else {
...
NetConnection::Platform::MakeConnectionFromExistingSocket(SOCKET sock,
                                                          uint32_t boundAddr,
                                                          uint16_t boundPort,
                                                          uint32_t peerAddr,
                                                          uint16_t peerPort) {
  const auto connection = std::make_shared<NetConnection>();
  connection->m_peerAddr = peerAddr;
  connection->m_peerPort = peerPort;
  connection->m_boundAddr = boundAddr;
  connection->m_boundPort = boundPort;
  return connection;
}